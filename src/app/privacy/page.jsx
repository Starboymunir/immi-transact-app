"use client"
import "./style.css";
import "../globals.css"
import HomepageFooter from "../HomepageFooter";
import Header from "../components/header.jsx";

const SecurityAndPrivacy = () => {
 

  return (
    <div className="security-and-privacy">
      <Header
                    className="header-instance"
                    img="material-symbols-notifications-outline-2.svg"
                    layer="layer-1-2.png"
                    maskGroup="image.png"
                    materialSymbols="material-symbols-search-2.svg"
                    memoryMessage="memory-message-2.svg"
                    tablerSettings="tabler-settings-2.svg"
                />
                <div className="div-1">
      <p><b className="privacy-policy">Privacy Policy</b></p>
      <p><b className="please-read-this">
        Please read this Privacy Policy carefully as it will help you make
        informed decisions about sharing your personal information with us.
      </b></p>
      <b className="introduction">Introduction:</b>
      <b className="what-this-privacy">What this Privacy Policy Covers:</b>
      <div className="immi-transact-is-thoroughly">
        Immi-Transact is thoroughly committed to protecting the privacy of our
        site visitors and customers. We fully appreciate and respect the
        importance of privacy on the Internet. We respect each visitors and
        customers right to personal privacy. To this end, we collect and use
        information throughout our website only as disclosed in this Privacy
        Policy.
      </div>
      <div className="this-policy-does">
        This policy does not apply to the practices of third party agents, or
        people of which we do not exercise direct control, employment or
        managements and.
      </div>
      <div className="consent-by-using-this-site-y-parent">
        <div className="consent-by-using-container">
          <p className="consent">
            <b>1. Consent:</b>
          </p>
          <p className="by-using-this">
            By using this site, you have consented to our Privacy Policy and
            that you have the legal capacity to give consent. Otherwise, you may
            discontinue and log out at this stage.
          </p>
        </div>
        <div className="consent-by-using-container">
          <p className="consent">
            <b>1. Consent:</b>
          </p>
          <p className="by-using-this">
            By using this site, you have consented to our Privacy Policy and
            that you have the legal capacity to give consent. Otherwise, you may
            discontinue and log out at this stage.
          </p>
        </div>
        <div className="consent-by-using-container">
          <p className="consent">
            <b>2. Lorem Ipsum:</b>
          </p>
          <p className="by-using-this">
            Lorem ipsum dolor sit amet, consectetur adipiscing elit. At eget
            iaculis eget eget neque, posuere quis placerat arcu. Lorem ipsum
            dolor sit amet, consectetur adipiscing elit. At eget iaculis eget
            eget neque, posuere quis placerat arcu. Lorem ipsum dolor sit amet,
            consectetur adipiscing elit. At eget iaculis eget eget neque,
            posuere quis placerat arcu. Lorem ipsum dolor sit amet, consectetur
            adipiscing elit. At eget iaculis eget eget neque, posuere quis
            placerat arcu.
          </p>
        </div>
        <div className="consent-by-using-container">
          <p className="consent">
            <b>3. Lorem Ipsum:</b>
          </p>
          <p className="by-using-this">
            Lorem ipsum dolor sit amet, consectetur adipiscing elit. At eget
            iaculis eget eget neque, posuere quis placerat arcu. Lorem ipsum
            dolor sit amet, consectetur adipiscing elit. At eget iaculis eget
            eget neque, posuere quis placerat arcu. Lorem ipsum dolor sit amet,
            consectetur adipiscing elit. At eget iaculis eget eget neque,
            posuere quis placerat arcu. Lorem ipsum dolor sit amet, consectetur
            adipiscing elit. At eget iaculis eget eget neque, posuere quis
            placerat arcu.
          </p>
        </div>
        <div className="consent-by-using-container">
          <p className="consent">
            <b>4. Lorem Ipsum:</b>
          </p>
          <p className="by-using-this">{`Lorem ipsum dolor sit amet, consectetur adipiscing elit. At eget iaculis eget eget neque, posuere quis placerat arcu. Lorem ipsum dolor sit amet, consectetur adipiscing elit. At eget iaculis eget eget neque, posuere quis placerat arcu. Lorem ipsum dolor sit amet, consectetur adipiscing elit. At eget iaculis eget eget neque, posuere quis placerat arcu. Lorem ipsum dolor sit amet, consectetur adipiscing elit. At eget iaculis eget eget neque, posuere quis placerat arcu. Lorem ipsum dolor sit amet, consectetur adipiscing elit. At eget iaculis eget eget neque, posuere quis placerat arcu. Lorem ipsum dolor sit amet, consectetur adipiscing elit. At eget iaculis eget eget neque, posuere quis placerat arcu. Lorem ipsum dolor sit amet, consectetur adipiscing elit. At eget iaculis eget eget neque, posuere quis placerat arcu. Lorem ipsum dolor sit amet, consectetur adipiscing elit. At eget iaculis eget eget neque, posuere quis placerat arcu. `}</p>
        </div>
        <div className="consent-by-using-container">
          <p className="consent">
            <b>5. Lorem Ipsum:</b>
          </p>
          <p className="by-using-this">
            Lorem ipsum dolor sit amet, consectetur adipiscing elit. At eget
            iaculis eget eget neque, posuere quis placerat arcu. Lorem ipsum
            dolor sit amet, consectetur adipiscing elit.
          </p>
        </div>
        <div className="consent-by-using-container">
          <p className="consent">
            <b>6. Lorem Ipsum:</b>
          </p>
          <p className="by-using-this">
            Lorem ipsum dolor sit amet, consectetur adipiscing elit. At eget
            iaculis eget eget neque, posuere quis placerat arcu. Lorem ipsum
            dolor sit amet, consectetur adipiscing elit. At eget iaculis eget
            eget neque, posuere quis placerat arcu. Lorem ipsum dolor sit amet,
            consectetur adipiscing elit. At eget iaculis eget eget neque,
            posuere quis placerat arcu. Lorem ipsum dolor sit amet, consectetur
            adipiscing elit. At eget iaculis eget eget neque, posuere quis
            placerat arcu.
          </p>
        </div>
        <div className="lorem-ipsum-a-container">
          <p className="lorem-ipsum5">
            <b>7. Lorem Ipsum:</b>
          </p>
          <p className="a-your-personal">Lorem ipsum dolor sit amet:</p>
          <p className="a-your-personal">
            a. Your personal data on Konga.com shall be collected and processed
            in accordance with legitimate and lawful purpose consented to by you
            herein, provided that a further processing may be done only for
            archiving, scientific research, historical research or statistical
            purposes for public interest in line with extant regulations and
            laws.
          </p>
          <p className="a-your-personal">
            b. Personal data shall be adequate, accurate and without prejudice
            to the dignity of human person:
          </p>
          <p className="a-your-personal">
            c. Personal data shall be stored only for the period within which it
            is reasonably needed, and shall be secured against all foreseeable
            hazards and breaches such as theft, cyberattack, viral attack,
            dissemination, manipulations of any kind, damage by rain, fire or
            exposure to other natural elements.
          </p>
          <p className="a-your-personal">
            d. Our data collection shall depict that we owe a duty of care to
            you.
          </p>
          <p className="a-your-personal">
            e. We shall be accountable in line with extant laws and regulations.
          </p>
        </div>
        <div className="consent-by-using-container">
          <p className="consent">
            <b>6. Lorem Ipsum:</b>
          </p>
          <p className="by-using-this">
            Lorem ipsum dolor sit amet, consectetur adipiscing elit. At eget
            iaculis eget eget neque, posuere quis placerat arcu. Lorem ipsum
            dolor sit amet, consectetur adipiscing elit. At eget iaculis eget
            eget neque, posuere quis placerat arcu. Lorem ipsum dolor sit amet,
            consectetur adipiscing elit. At eget iaculis eget eget neque,
            posuere quis placerat arcu. Lorem ipsum dolor sit amet, consectetur
            adipiscing elit. At eget iaculis eget eget neque, posuere quis
            placerat arcu.
          </p>
        </div>
        <div className="lorem-ipsum-a-container">
          <p className="lorem-ipsum5">
            <b>7. Lorem Ipsum:</b>
          </p>
          <p className="a-your-personal">
            a. Your personal data on Konga.com shall be collected and processed
            in accordance with legitimate and lawful purpose consented to by you
            herein, provided that a further processing may be done only for
            archiving, scientific research, historical research or statistical
            purposes for public interest in line with extant regulations and
            laws.
          </p>
          <p className="a-your-personal">
            b. Personal data shall be adequate, accurate and without prejudice
            to the dignity of human person:
          </p>
          <p className="a-your-personal">
            c. Personal data shall be stored only for the period within which it
            is reasonably needed, and shall be secured against all foreseeable
            hazards.
          </p>
        </div>
        <div className="consent-by-using-container">
          <p className="consent">
            <b>6. Lorem Ipsum:</b>
          </p>
          <p className="by-using-this">
            Lorem ipsum dolor sit amet, consectetur adipiscing elit. At eget
            iaculis eget eget neque, posuere quis placerat arcu. Lorem ipsum
            dolor sit amet, consectetur adipiscing elit. At eget iaculis eget
            eget neque, posuere quis placerat arcu. Lorem ipsum dolor sit amet,
            consectetur adipiscing elit. At eget iaculis eget eget neque.
          </p>
        </div>
        <div className="consent-by-using-container">
          <p className="consent">
            <b>6. Lorem Ipsum:</b>
          </p>
          <p className="by-using-this">
            Lorem ipsum dolor sit amet, consectetur adipiscing elit. At eget
            iaculis eget eget neque, posuere quis placerat arcu. Lorem ipsum
            dolor sit amet, consectetur adipiscing elit. At eget iaculis eget
            eget neque, posuere quis placerat arcu. Lorem ipsum dolor sit amet,
            consectetur adipiscing elit. At eget iaculis eget eget neque,
            posuere quis placerat arcu. Lorem ipsum dolor sit amet, consectetur
            adipiscing elit. At eget iaculis eget eget neque, posuere quis
            placerat arcu. Lorem ipsum dolor sit amet, consectetur adipiscing
            elit. At eget iaculis eget eget neque, posuere quis placerat arcu.
            Lorem ipsum dolor sit amet, consectetur adipiscing elit. At eget
            iaculis eget eget neque, posuere quis placerat arcu. Lorem ipsum
            dolor sit amet, consectetur adipiscing elit. At eget iaculis eget
            eget neque, posuere quis placerat arcu. Lorem ipsum dolor sit amet,
            consectetur adipiscing elit. At eget iaculis eget eget neque,
            posuere quis placerat arcu.Lorem ipsum dolor sit amet, consectetur
            adipiscing elit. At eget iaculis eget eget neque, posuere quis
            placerat arcu. Lorem ipsum dolor sit amet, consectetur adipiscing
            elit. At eget iaculis eget eget neque, posuere quis placerat arcu.
            Lorem ipsum dolor sit amet, consectetur adipiscing elit. At eget
            iaculis eget eget neque, posuere quis placerat arcu. Lorem ipsum
            dolor sit amet, consectetur adipiscing elit. At eget iaculis eget
            eget neque, posuere quis placerat arcu.
          </p>
        </div>
      </div>
      <div className="how-we-collect-container">
      </div>
      </div>
      <HomepageFooter
                    asset="asset-9ldpi-1-2.svg"
                    className="homepage-footer-instance"
                    phCopyright="ph-copyright-2.svg"
                    to="/faq"
                />
    </div>
  );
};

export default SecurityAndPrivacy;
